  ...
  setup(props, {emit}) {
    ...
    const initSource = (resetData) => {
      ...
      props.formRules.forEach((item) => {
        item.forEach((x) => {
          if (resetData||( x.dataKey && (!x.data || x.data.length == 0) && !x.remote)) {
            x.data = [];
    ...
    const changeGroup = () => {
      ...
      proxy.$emit("tabClick",currentGroup.value)
    };
        ...
        return {
          validator: (rule, val, callback) => {
            ...
            if (item.required && !this.isReadonly(item) && (!val || !val.length)) {
              return callback(
                new Error(this.$ts(item.type == "img" ? "请上传照片" : "请上传文件"))
              );
            }
            return callback();
          },

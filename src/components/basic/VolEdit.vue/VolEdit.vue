<template>
  <div class="edit-container">
    ...
    <el-scrollbar style="flex: 1; height: 0; padding: 12px">
      ...
      <div class="detail-table">
        <detail-table
          ...
          @loadAfter="loadDSubetailTableAfter"
          @rowChange="() => {}"
          @rowClick="rowClick"
          @tabsClick="() => {}"
    ...
    <ViewGridAudit @auditClick="saveAudit" :option="autitTableOptions" ref="audit">
    </ViewGridAudit>
  </div>
</template>
<script>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    //二级表配置
    if (proxy.details.length) {
      emit("initDetailColumns", proxy.details);
    }
    //三级表配置
